...
export function getPossibleMovesKing(position, board) {
    ...
    castleRight = (castleRight.length > 0) ? castleRight[castleRight.length - 1] : null

    if (castleLeft)
        castleLeft = ('moves' in board[castleLeft.y][castleLeft.x]) ? null : castleLeft
    if (castleRight)
        castleRight = ('moves' in board[castleRight.y][castleRight.x]) ? null: castleRight
